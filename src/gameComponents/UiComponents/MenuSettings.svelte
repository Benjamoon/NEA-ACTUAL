<script>
    ...
    const save = async () => {
        ...
        if (result.ok) {
            // If it was ok, hide the loader and disable saving (because we have just saved)
    ...
    onMount(async () => {
        ...
        if (response.ok == true) {
            ...
            if (response.settings == false) {
                // user had no settings so keep defaults
                settingsLoading = false;
                return;
            }
...
<Card centered="true">
    ...
    <CardContent>
        ...
        {#each settings as setting}
            <Button
                success={settingsValues[setting.value]}
                text="{setting.label} | {settingsValues[setting.value]
                    ? 'Enabled'
                    : 'Disabled'}"
                on:click={() => {
